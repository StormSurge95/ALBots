import AL, { BankPackName, Character, Constants, Entity, GameResponseData, GMap, HitData, IEntity, InviteData, IPosition, ItemData, ItemName, ItemType, MapName, Merchant, MonsterName, NPCName, Pathfinder, Player, ServerIdentifier, ServerInfoDataLive, ServerInfoDataNotLive, ServerRegion, SlotType, Tools, TradeSlotType } from "alclient"
...
export const MY_CHARACTERS = ["StormSurge", "WarriorSurge", "PriestSurge", "RangerSurge", "MageSurge", "PaladinSurge"]
...
/* Set of items to hold onto so they don't get sold/listed/trashed */
...
/* Set of items to buy from ponty if they happen to show up */
...
export const ITEMS_TO_DISMANTLE: ItemLevelInfo = {
    "molesteeth": 2
}

/* "Item to sell": "maximum level to sell"
 * If an item is at or below the level listed here, it will be sold
 */
export const ITEMS_TO_SELL: ItemLevelInfo = {
    // Things that accumulate
    "cclaw": 2, "frankypants": 2, "hpamulet": 2, "hpbelt": 2, "quiver": 2,
    "ringsj": 2, "slimestaff": 2, "stinger": 2, "vitearring": 2, "vitring": 2,
    "elixirdex0": 999, "elixirstr0": 999, "elixirint0": 999, "cape": 2,
    "maceofthedead": 2, "daggerofthedead": 2, "bowofthedead": 2, "swordofthedead": 2,
    "staffofthedead": 2, "pmaceofthedead": 2, "spearofthedead": 2,
    "swifty": 2, "t2bow": 2, "throwingstars": 2, "dagger": 2,
    "sword": 2, "spear": 2,
    "stramulet": 0, "intamulet": 0, "dexamulet": 0,
    "strbelt": 0, "intbelt": 0, "dexbelt": 0,
    "strearring": 0, "intearring": 0, "dexearring": 0,
    "strring": 0, "intring": 0, "dexring": 0,
    "fireblade": 0, "firestaff": 0, "vitscroll": 999,
    "mushroomstaff": 2, "shield": 2, "sshield": 2, "wbook0": 2,
    "hbow": 2,
...
export const ITEMS_TO_COMPOUND: Set<ItemName> = new Set([
    "t2stramulet", "t2dexamulet", "t2intamulet",
    "wbook1", "rabbitsfoot", "jacko"
])
...
export const ITEMS_TO_PRIMLING: ItemLevelInfo = {
    ...
    "harmor": 4, "hboots": 4, "hgloves": 4, "hhelmet": 4, "hpants": 4,
...
export const ITEM_HOLD_LIMIT: { [T in ItemName]?: number } = {
    wcap: 2,
    wattire: 2,
    wgloves: 2,
    wbreeches: 2,
...
export const UPGRADE_COMPOUND_LIMIT: ItemLevelInfo = {
    ...
    "vorb": 0, // No advanatages for leveling this item
...
/**
 * calculates the cooldown for the attack loop of a particular bot character
 * based on various skill cooldowns
 * @param bot The bot to calculate the cooldown for
 * @returns the calculated cooldown value
 */
...
/**
 * Gets the first empty inventory slot and returns its index
 * @param items The list of items to search (ie: `bot.items` or bank slots)
 * @returns The index of the first empty slot within the list of items searched
 */
...
/**
 * Ensures that a bot has a particular item equipped in a particular slot.
 * If the item is held but not equipped, this function equips it. If the
 * item is not held and not equipped, this function fails and returns false.
 * @param bot The bot to equip the item
 * @param item The item to equip
 * @param slot The slot to equip the item in
 * @returns whether or not the item is equipped.
 */
...
export async function getPriority1Entities(bot: Character): Promise<Entity[] | IEntity[]> {
    ...
    for (const key in bot.S) {
        ...
        if (typeof data == "object" && (data as ServerInfoDataLive | ServerInfoDataNotLive).live) {
            if (!(data as ServerInfoDataLive).target
                && !["grinch"].includes(key) /** Grinch changes target a lot, keep on him */) continue // No target
            alive.push({
                in: (data as ServerInfoDataLive).map,
                map: (data as ServerInfoDataLive).map,
                serverIdentifier: bot.server.name,
                serverRegion: bot.server.region,
                type: key as MonsterName,
                x: (data as ServerInfoDataLive).x,
                y: (data as ServerInfoDataLive).y,
            })
        }
    }
    return alive
...
/**
 * Gathers and returns a list of monsterhunt targets for everyone within the current party/friends list.
 *
 * The gathered list is sorted based on how much time is left; less -> higher priority
 * @param bot
 * @param friends
 * @returns
 */
...
export async function goToSpecialMonster(bot: Character, type: MonsterName, options: { requestMagiport?: true } = {}): Promise<unknown> {
    ...
    if (bot.S && bot.S[type] && (bot.S[type] as ServerInfoDataLive | ServerInfoDataNotLive).live && bot.S[type]["x"] !== undefined && bot.S[type]["y"] !== undefined) {
    ...
    if ((dbTarget && dbTarget.x == undefined && dbTarget.y == undefined && dbTarget.map)
        || (bot.S && bot.S[type] && (bot.S[type] as ServerInfoDataLive | ServerInfoDataNotLive).live && bot.S[type]["x"] !== undefined && bot.S[type]["y"] !== undefined)) {
...
/**
 * Go to the potion seller NPC if we're low on potions so we can by some
 *
...
export async function goToPotionSellerIfLow(bot: Character, minHpPots = 100, minMpPots = 100): Promise<void> {
    if (bot.hasItem("computer") || bot.hasItem("supercomputer")) return // Don't need to move if we have a computer
...
export async function moveInCircle(bot: Character, center: IPosition, radius = 125, angle = Math.PI / 2.5): Promise<IPosition> {
    if (AL.Pathfinder.canWalkPath(bot, center)) {
        ...
        bot.move(endGoal.x, endGoal.y, { resolveOnStart: true, disableErrorLogs: true }).catch(() => { /* Suppress errors */ })
    } else {
        // Move to where we can walk
        if (bot.targets > 0) {
            return bot.smartMove(center, { getWithin: radius / 2, avoidTownWarps: true })
        } else {
            return bot.smartMove(center, { getWithin: radius / 2 })
        }
    }
}

export async function moveInEllipse(bot: Character, center: IPosition, height = 125, width = 125, angle = Math.PI / 2.5): Promise<IPosition> {
    if (AL.Pathfinder.canWalkPath(bot, center)) {
        const angleFromCenterToCurrent = Math.atan2(bot.y - center.y, bot.x - center.x)
        const endGoalAngle = angleFromCenterToCurrent + angle
        const endGoal = { x: center.x + (width / 2) * Math.cos(endGoalAngle), y: center.y + (height / 2) * Math.sin(endGoalAngle) }
        bot.move(endGoal.x, endGoal.y, { resolveOnStart: true }).catch(() => { /* Suppress errors */ })
    } else {
        const dist = (height < width) ? height : width
        return bot.smartMove(center, { getWithin: dist })
    }
}
            ...
            if (Date.now() > lastPonty + CHECK_PONTY_EVERY_MS) {
                for (const ponty of pontyLocations) {
                    ...
                    for (const item of pontyItems) {
                        if (!item) continue
                        if (!itemsToBuy.has(item.name)) continue
                        const price = bot.G.items[item.name].g * Constants.PONTY_MARKUP * (item.q ? item.q : 1)
                        const gold = bot.gold / 2
                        if (price > gold) continue
                        const count = bot.countItem(item.name)
                        const limit = ITEM_HOLD_LIMIT[item.name] ?? Number.MAX_SAFE_INTEGER
            ...
            for (const [, player] of bot.players) {
                ...
                for (const s in player.slots) {
                    ...
                    if (bot.gold >= item.price &&
                        ...
                        || (itemsToBuy.has(item.name) && !buyableFromNPC && item.price <= cost * AL.Constants.PONTY_MARKUP))
                    ) {
...
export function startBuyFriendsReplenishablesLoop(bot: Character, friends: Character[], replenishablesToBuy = REPLENISHABLES_TO_BUY): void {
    async function buyFriendsReplenishablesLoop() {
        try {
            ...
            if (!(bot.hasItem("computer") || bot.hasItem("supercomputer"))) return // We can't buy potions anywhere, don't run this loop
            ...
            for (let i = 0; i < friends.length; i++) {
                ...
                if (friend.hasItem("computer") || friend.hasItem("supercomputer")) continue // They can buy their own potions.
...
export function startCompoundLoop(bot: Character, itemsToSell: ItemLevelInfo = ITEMS_TO_SELL, itemsToCompound: Set<ItemName> = new Set([])): void {
    async function compoundLoop() {
        try {
            ...
            if (Tools.distance(bot, { map: "main", x: -180, y: -185 }) > Constants.NPC_INTERACTION_DISTANCE / 2) {
                bot.timeouts.set("upgradeLoop", setTimeout(async () => { compoundLoop() }, LOOP_MS))
                return
            }
            ...
            for (const dName in itemsByLevel) {
                ...
                if (itemsToCompound.size > 0 && !itemsToCompound.has(itemName)) continue
                ...
                for (let dLevel = 7; dLevel >= 0; dLevel--) {
                    ...
                    const grade = bot.calculateItemGrade({ level: dLevel, name: itemName })
                    ...
                    if (dLevel >= 4 - level0Grade) {
                        ...
                        for (let i = 0; i < items.length; i++) {
                            ...
                            if (dLevel <= itemsToSell[itemName]) break // We don't want to compound items we want to sell

                            let cscrollPos = bot.locateItem(cscrollName)
                            const primlingPos = bot.locateItem("offeringp")
                            try {
                                if (cscrollPos == undefined && !bot.canBuy(cscrollName)) break // We can't buy a scroll for whatever reason :(
                                ...
                                if ((ITEMS_TO_PRIMLING[itemName] && dLevel >= ITEMS_TO_PRIMLING[itemName])
                                    ...
                                    res = await bot.compound(items[0], items[1], items[2], cscrollPos, primlingPos)
                                } else {
                                    // We don't want to use an offeringp to upgrade these
                                    if (!bot.s.massproduction && bot.canUse("massproduction")) (bot as Merchant).massProduction()
                                    res = await bot.compound(items[0], items[1], items[2], cscrollPos)
                                }
                                if (res) console.log(`Successfully compounded ${item.name} to level ${item.level + 1}`)
                                else console.log(`Compound failed; level ${item.level} ${item.name} x3 lost`)
                                i += 2
                            } catch (e) {
                                console.error(e)
                            }
...
export async function startDismantleLoop(bot: Character, itemsToDismantle: ItemName[]) {
    async function dismantleLoop() {
        try {
            if (!bot.socket || bot.socket.disconnected) return

            for (const iName in itemsToDismantle) {
                console.log(iName)
                // TODO: dismantle function
            }
        } catch (e) {
            console.error(e)
        }
        bot.timeouts.set("dismantleLoop", setTimeout(async () => { dismantleLoop() }, 1000))
    }
    dismantleLoop()
}
...
export function startExchangeLoop(bot: Character, itemsToExchange = ITEMS_TO_EXCHANGE): void {
    async function exchangeLoop() {
        try {
            if (!bot.socket || bot.socket.disconnected) {
                console.log(`${bot.id} is disconnected! Exiting exchangeLoop!`)
                return
            }

            if (bot.q.exchange) {
                // We are exchanging, we have to wait
                console.log(`Currently exchanging ${bot.q.exchange.id}; waiting ${bot.q.exchange.ms}ms...`)
                bot.timeouts.set("exchangeLoop", setTimeout(async () => { exchangeLoop() }, bot.q.exchange.ms))
                return
            }

            if (bot.esize > 3 /* Only exchange if we have plenty of space */
                ...
                for (let i = 0; i < bot.items.length; i++) {
                    ...
                    if (!itemsToExchange.has(item.name)) {
                        continue // Don't want/can't exchange
                    }
                    if (!bot.canExchange(item.name)) {
                        continue // Can't exchange.
                    }
                    console.log(`Exchanging ${item.name}!`)
            ...
            if (!(bot.G.maps[bot.map] as GMap).mount /* Don't exchange in the bank */
                ...
                const lers = bot.locateItems("lostearring", bot.items, { level: 2 })
                for (let i = 0; i < lers.length; i++) {
                    await bot.exchange(i)
                }
            }
        } catch (e) {
    ...
    async function lootLoop() {
        try {
            ...
            for (const [id, chest] of bot.chests) {
                ...
                const data = await bot.openChest(id)
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                for (const loot of (data as any).items || []) {
                    console.log("%c%s looted %c%sx%i", "color: yellow;", loot.looter, "color: red;", loot.name, loot.q ?? 1)
                }
                lootLoop()
                return
            }
            ...
            for (const [id, chest] of bot.chests) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                for (const loot of (data as any).items || []) {
                    console.log(`${loot.looter} looted ${loot.name}x${loot.q ?? 1}`)
                }
                lootLoop()
                return
            }
                    ...
                    if (bot.partyData && bot.partyData.list.length >= 9) {
                        ...
                        for (let i = bot.partyData.list.indexOf(bot.id) + 1; i < bot.partyData.list.length; i++) {
                            ...
                            if (!partyMembers.includes(memberName)) {
                                // Someone snuck into our party
                                toKickMember = memberName
                                break
                            }

                            const memberPriority = partyMembers.length - partyMembers.indexOf(memberName)
                            if (memberPriority > toKickPriority) continue // This member has higher priority
    ...
    async function scareLoop() {
        try {
            ...
            if (bot.canUse("scare", { ignoreEquipped: true })
                ...
                await bot.scare().catch((e) => { console.error(e) })
        ...
        if (typeof data == "string") {
            if (data == "cant_escape") {
                if (bot.isScared() || bot.targets >= 5) {
                    ...
                    // Scare, because we are scared
                    bot.scare().catch((e) => { console.error(e) })
        ...
        try {
            ...
            if (bot.canSell()) {
                ...
                for (let i = 0; i < bot.items.length; i++) {
                    ...
                    if (bot.countItem(item.name) < ITEM_HOLD_LIMIT[item.name] ?? Number.MAX_VALUE) continue

                    if (item.q && ITEM_HOLD_LIMIT[item.name]) {
                        const count = item.q - ITEM_HOLD_LIMIT[item.name]
                        if (count > 0) await bot.sell(i, item.q - ITEM_HOLD_LIMIT[item.name])
                    } else await bot.sell(i, item.q ?? 1)
                }
            }
 ...
 * @param goldToHold
 */
export function startSendStuffDenyListLoop(bot: Character, sendTo: string[], itemsToHold = ITEMS_TO_HOLD, goldToHold = GOLD_TO_HOLD): void {
    async function sendStuffLoop() {
        try {
            if (!bot.socket || bot.socket.disconnected) return
...
export function startUpgradeLoop(bot: Character, itemsToSell: ItemLevelInfo = ITEMS_TO_SELL, itemsToUpgrade: Set<ItemName> = new Set([])): void {
    async function upgradeLoop() {
        try {
            ...
            if (Tools.distance(bot, { map: "main", x: -236, y: -185 }) > Constants.NPC_INTERACTION_DISTANCE / 2) {
                bot.timeouts.set("upgradeLoop", setTimeout(async () => { upgradeLoop() }, LOOP_MS))
                return
            }
            ...
            for (const dName in itemsByLevel) {
                const itemName = dName as ItemName
                if (itemsToUpgrade.size > 0 && !itemsToUpgrade.has(itemName)) continue
                const gInfo = bot.G.items[itemName]
                        ...
                        for (let i = 0; i < items.length; i++) {
                            ...
                            try {
                                ...
                                if ((ITEMS_TO_PRIMLING[itemName] && dLevel >= ITEMS_TO_PRIMLING[itemName])
                                    ...
                                    if (!bot.s.massproduction && bot.canUse("massproduction")) (bot as Merchant).massProduction().catch(() => { /* ignore errors */ })
                                    res = await bot.upgrade(slot, scrollPos, primlingPos)
                                } else {
                                    // We don't want to use an offeringp to upgrade these
                                    if (!bot.s.massproduction && bot.canUse("massproduction")) (bot as Merchant).massProduction().catch(() => { /* ignore errors */ })
                                    res = await bot.upgrade(slot, scrollPos)
                                }
                                if (res) console.log(`Successfully upgraded ${item.name} to level ${item.level + 1}`)
                                else console.log(`Failed to upgrade; level ${item.level} ${item.name} lost`)
                            } catch (e) {
                                console.error(e)
                            }
                        }
...
export function getTotalTargets(bot: Character) {
    const targets = bot.calculateTargets()

    let total = 0

    if (!bot) return 0
    if (!bot.socket || bot.socket.disconnected) return 0

    for (const targ in targets) {
        total += targets[targ][0]
    }
...
export async function sortInventory(bot: Character, items: ItemData[] = bot.items, bankPack: BankPackName = undefined): Promise<void> {
    const data: SortData[] = []
...
export async function sortBank(bot: Character): Promise<void> {
    ...
    for (let i = 0; i < 7; i++) {
        const pack = `items${i}` as Exclude<BankPackName, "gold">
        if (!bot?.bank[pack]) continue
        const packItems = bot.bank[pack]
        for (let i = 0; i < packItems.length; i++) {
            const item = packItems[i]
            if (!item) continue
            const gItem = bot.G.items[item.name]
            items.push({ id: i, currentI: i, fromPack: pack, item: item, type: types[gItem.type], tier: gItem.tier })
        }
    }
    ...
    const fromPacks = [
        items.filter(a => a.fromPack == "items0" && a.toPack !== "items0"),
        items.filter(a => a.fromPack == "items1" && a.toPack !== "items1"),
        items.filter(a => a.fromPack == "items2" && a.toPack !== "items2"),
        items.filter(a => a.fromPack == "items3" && a.toPack !== "items3"),
        items.filter(a => a.fromPack == "items4" && a.toPack !== "items4"),
        items.filter(a => a.fromPack == "items5" && a.toPack !== "items5"),
        items.filter(a => a.fromPack == "items6" && a.toPack !== "items6")
    ]
    ...
    for (let i = 0; i < 7; i++) {
        ...
        await sortInventory(bot, bot.bank[pack], pack)
    }
}