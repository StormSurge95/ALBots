...
export async function attackTheseTypesMage(bot: Mage, types: MonsterName[], friends: Character[] = [], options: {
    ...
    if (bot.canUse("attack")) {
        ...
        if (!bot.s.energized && !options.disableEnergize) {
            for (const friend of friends) {
                ...
                // Energize!
                (friend as Mage).energize(bot.id, Math.min(100, Math.max(1, bot.max_hp - bot.mp))).catch(e => console.error(e))
                break
            }
        }
    ...
    if (bot.canUse("cburst") && !options.disableCburst && bot.mp > bot.max_hp - 500) {
        const targets = new FastPriorityQueue<Entity>(priority)
        for (const entity of bot.getEntities({
            couldGiveCredit: true,
            typeList: types,
