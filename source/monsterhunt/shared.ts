import AL, { Character, CMData, Constants, Entity, IPosition, ItemName, LimitDCReportData, Mage, Merchant, MonsterName, Paladin, Priest, Ranger, Rogue,
    ServerIdentifier, ServerRegion, SlotType, Tools, Warrior } from "alclient"
import { calculateAttackLoopCooldown, checkOnlyEveryMS, getMonsterHuntTargets, getPriority1Entities, getPriority2Entities, goGetRspeedBuff, goToNearestWalkableToMonster2,
    ITEMS_TO_BUY, ITEMS_TO_HOLD, ITEMS_TO_LIST, LOOP_MS, REPLENISHABLES_TO_BUY, sleep, startAvoidStacking, startBuyFriendsReplenishablesLoop, startBuyLoop, startCompoundLoop,
    startCraftLoop, startElixirLoop, startExchangeLoop, startHealLoop, startLootLoop, startPartyLoop, startScareLoop, startSellLoop, startSendStuffDenylistLoop, startUpgradeLoop } from "../base/general.js"
...
function getTgt(id: string, information: Information): MonsterName {
    if (id == information.bot1.name) {
        return information.bot1.target
    }
    if (id == information.bot2.name) {
        return information.bot2.target
    }
    if (id == information.bot3.name) {
        return information.bot3.target
    }
    return null
}

async function handleEquipment(bot: Character, target: MonsterName, strategy: Strategy): Promise<void> {
    if (strategy[target].equipment) {
        for (const s in strategy[target].equipment) {
            ...
            if (bot.slots[slot]?.name == itemName) {
                // We already have it euqipped, see if there's a higher level item to equip
...
export async function startMage(bot: Mage, information: Information, strategy: Strategy, partyLeader: string, partyMembers: string[], defaultRegion = DEFAULT_REGION, defaultIdentifier = DEFAULT_IDENTIFIER): Promise<void> {
    ...
    const idleTargets = getIdles(strategy)
    ...
    async function attackLoop() {
        try {
            ...
            const target = getTgt(bot.id, information)

            if (target) {
                // Equipment
                await handleEquipment(bot, target, strategy)
    ...
    async function attackLoop() {
        try {
            ...
            if (information.merchant.target
                ...
                // Equipment
                await handleEquipment(bot, information.merchant.target, strategy)
            ...
            for (const friend of information.friends) {
                ...
                && (bot.hasItem("computer") || bot.hasItem("supercomputer"))) {
                    ...
                    for (const [item, amount] of REPLENISHABLES_TO_BUY) {
                        ...
                        if (!bot.canBuy(item)) continue // We can't buy them this item
...
export async function startPriest(bot: Priest, information: Information, strategy: Strategy, partyLeader: string, partyMembers: string[], defaultRegion = DEFAULT_REGION, defaultIdentifier = DEFAULT_IDENTIFIER): Promise<void> {
    ...
    async function attackLoop() {
        try {
            ...
            if (target && !bot.isOnCooldown("scare")) {
                // Equipment
                await handleEquipment(bot, target, strategy)
...
export async function startShared(bot: Character, strategy: Strategy, information: Information, partyLeader: string, partyMembers: string[], defaultRegion = DEFAULT_REGION, defaultIdentifier = DEFAULT_IDENTIFIER): Promise<void> {
    ...
    if (bot.ctype == "merchant") {
        ...
        startSendStuffDenylistLoop(bot, [information.merchant.name], ITEMS_TO_HOLD, 10_000_000)
    }
    ...
    async function targetLoop(): Promise<void> {
        try {
            ...
            if (bot.id == information.bot1.name) {
                if (newTarget !== information.bot1.target) {
                    bot.stopSmartMove().catch(() => { /** Suppress */ })
                    console.log(`changing ${information.bot1.name}'s target from ${information.bot1.target} to ${newTarget}`)
                }
                information.bot1.target = newTarget
            } else if (bot.id == information.bot2.name) {
                if (newTarget !== information.bot2.target) {
                    bot.stopSmartMove().catch(() => { /** Suppress */ })
                    console.log(`changing ${information.bot2.name}'s target from ${information.bot2.target} to ${newTarget}`)
                }
                information.bot2.target = newTarget
            } else if (bot.id == information.bot3.name) {
                if (newTarget !== information.bot3.target) {
                    bot.stopSmartMove().catch(() => { /** Suppress */ })
                    console.log(`changing ${information.bot3.name}'s target from ${information.bot3.target} to ${newTarget}`)
                }
                information.bot3.target = newTarget
            } else if (bot.id == information.merchant.name) {
                if (newTarget !== information.merchant.target) {
                    bot.stopSmartMove().catch(() => { /** Suppress */ })
                    console.log(`changing ${information.merchant.name}'s target from ${information.merchant.target} to ${newTarget}`)
                }
