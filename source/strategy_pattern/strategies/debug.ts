    ...
    public onApply(bot: Type) {
        ...
        if (this.options.logAttacks) {
            this.logAttacks = (name: string, data: unknown) => {
                ...
                console.debug(`[${this.getTimestamp()}] [${bot.id}] [logAttacks] ${JSON.stringify(data, undefined, 4)}`)
            }
        ...
        if (this.options.logEquips) {
            this.logEquips = (name: string, data: unknown) => {
                ...
                console.debug(`[${this.getTimestamp()}] [${bot.id}] [logEquips] ${JSON.stringify(data, undefined, 4)}`)
            }
        ...
        if (this.options.logPenalties) {
            this.logPenalty = (name: string, data: unknown) => {
                ...
                if ((data as any).penalty) {
                    console.debug(`[${this.getTimestamp()}] [${bot.id}] [logPenalties] [${name}: ${JSON.stringify(data, undefined, 4)}]`)
                }
            }
        ...
        if (this.options.logSkills) {
            this.logSkills = (name: string, data: ClientToServerSkillData) => {
                ...
                console.debug(`[${this.getTimestamp()}] [${bot.id}] [logSkills] ${JSON.stringify(data, undefined, 4)}`)
            }
        ...
        if (this.options.logSkillTimeouts) {
            this.logSkillTimeouts = (data: SkillTimeoutData) => {
                console.log(`[${this.getTimestamp()}] [${bot.id}] [logSkillTimeouts] ${JSON.stringify(data, undefined, 4)}`)
            }
