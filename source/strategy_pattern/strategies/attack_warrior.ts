import { ItemData, Tools, Warrior } from "../../../../ALClient/build/index.js"
...
export type WarriorAttackStrategyOptions = BaseAttackStrategyOptions & {
    ...
    disableWarCry?: true
    /**
     * If true, we will swap weapons to one that can cleave, cleave, then swap back.
     *
...
export class WarriorAttackStrategy extends BaseAttackStrategy<Warrior> {
    ...
    public constructor(options?: WarriorAttackStrategyOptions) {
        ...
        this.loops.set("attack", {
            fn: async (bot: Warrior) => {
                if (this.shouldHardshell(bot)) await bot.hardshell().catch(bot.error)
        ...
        if (!this.options.disableCleave) this.interval.push("cleave")
        if (!this.options.disableWarCry) this.interval.push("warcry")
        if (!this.options.disableStomp) this.interval.push("stomp")
    }
    ...
    protected async agitateTargets(bot: Warrior) {
        ...
        if (this.options.enableGreedyAggro) {
            ...
            if (wantedEntity) return bot.agitate().catch(bot.error)
        }
    }
    ...
    protected async cleave(bot: Warrior) {
        ...
        for (const entity of entities) {
            ...
            if ((this.options.type && this.options.type !== entity.type)
        ...
        await bot.cleave().catch(bot.error)

        if (this.options.enableEquipForCleave) {
            // Re-equip items
            if (mainhand) {
                await bot.equip(bot.locateItem(mainhand.name, bot.items, { level: mainhand.level, special: mainhand.p }), "mainhand")
            } else {
    ...
    protected async stomp(bot: Warrior) {
        ...
        if (this.options.enableEquipForStomp) {
            if (!(bot.isEquipped(["basher", "wbasher"]))) {
                ...
                if (bot.slots.mainhand) mainhand = { ...bot.slots.mainhand }
                await bot.equip(bot.locateItem(["basher", "wbasher"], bot.items, { returnHighestLevel: true }), "mainhand")
                if (bot.s.penalty_cd) await sleep(bot.s.penalty_cd.ms) // Await the penalty cooldown so we can stomp right away
            }
        }

        await bot.stomp().catch(bot.error)
        ...
        if (this.options.enableEquipForStomp) {
            ...
            if (offhand) {
                await bot.equip(bot.locateItem(offhand.name, bot.items, { level: offhand.level, special: offhand.p, statType: offhand.stat_type }), "offhand")
            }
        }
    }
    ...
    protected async applyWarCry(bot: Warrior) {
        ...
        await bot.warcry().catch(bot.error)
    }
