import { ActionData, ActionDataRay, Entity, Mage, SlotType, Tools, TradeItemInfo, TradeSlotType } from "../../../../ALClient/build/index.js"
...
export class MageAttackStrategy extends BaseAttackStrategy<Mage> {
    ...
    public onApply(bot: Mage): void {
        ...
        if (!this.options.disableCburst && !this.options.disableKillSteal) {
            this.stealOnActionCburst = async (data: ActionData) => {
                ...
                if (Tools.distance(bot, target) > bot.G.skills.cburst.range) return // Too far away to cburst
    ...
    protected async cburstHumanoids(bot: Mage) {
        ...
        for (const entity of entities) {
            if (!entity.humanoid) continue // Entity isn't humanoid
    ...
    protected async cburstAttack(bot: Mage, priority: (a: Entity, b: Entity) => boolean) {
        ...
        let mpPool = (bot.mp - bot.G.skills.cburst.mp) / 2
        if (mpPool <= 0) return
        ...
        for (let i = 0; i < entities.length; i++) {
            ...
            const mpNeededToKill = entity.hp * 1.1 / bot.G.skills.cburst.ratio * Tools.damage_multiplier(bot.rpiercing - entity.resistance)
