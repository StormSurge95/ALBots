import { GMap, ItemName, MapName, Character, TradeSlotType, IPosition, ItemDataTrade, Pathfinder, Game, Constants, Tools } from "../../../../ALClient/build/index.js"
...
export class BuyStrategy<Type extends Character> implements Strategy<Type> {
    ...
    public constructor(options: BuyStrategyOptions = RecommendedBuyStrategyOptions) {
        ...
        this.pontyLocations = Pathfinder.locateNPC("secondhands")

        for (const [itemName, buyFor] of this.buyMap ?? []) {
            const gItem = Game.G.items[itemName]
            ...
            for (const map in Game.G.maps) {
                if (buyableFromNPC) break
                if (Game.G.maps[map as MapName].ignore) continue
                for (const npc of (Game.G.maps[map as MapName] as GMap).npcs) {
                    if (buyableFromNPC) break
                    if (Game.G.npcs[npc.id].items == undefined) continue
                    buyableFromNPC = Game.G.npcs[npc.id].items.includes(itemName)
                    if (buyableFromNPC) break
                }
            }
    ...
    private async buyReplenishablesFromMerchants(bot: Type) {
        ...
        for (const player of players) {
            for (const s in player.slots) {
                ...
                if (item.price > bot.G.items[item.name].g) continue // They're selling it for more than it's worth

                const num = bot.countItem(item.name)
                const numToBuy = Math.min(amount - num, Math.floor(bot.gold / item.price))
    ...
    private async buyReplenishablesFromNPCs(bot: Type) {
        for (const [item, amount] of this.replenishables) {
            ...
            const numToBuy = Math.min(amount - num, Math.floor(bot.gold / bot.G.items[item].g))
            if (numToBuy <= 0) continue // We don't want to buy any
    ...
    private async buyFromPonty(bot: Type) {
        if (this.pontyLocations.every((location) => {
            return Tools.distance(bot, location) > Constants.NPC_INTERACTION_DISTANCE
        ...
        for (const pontyItem of this.pontyItems) {
            ...
            const pontyPrice = bot.G.items[pontyItem.name].g * Constants.PONTY_MARKUP // TODO: Use G.multipliers.secondhands_mult if it ever gets fixed
    ...
    private async buyFromMerchants(bot: Type) {
        ...
        for (const player of players) {
            for (const s in player.slots) {
                ...
                if (item.price > bot.gold) continue

                const willingToPay = this.buyMap.get(item.name)
                if (willingToPay === undefined) continue
                if (item.price > willingToPay) continue
    ...
    private async buyFromMerchantsForProfit(bot: Type) {
        ...
        for (const player of players) {
            for (const s in player.slots) {
                ...
                const npcValue = bot.G.items[item.name].g * bot.G.multipliers.buy_to_sell
                if (item.price > npcValue) continue
