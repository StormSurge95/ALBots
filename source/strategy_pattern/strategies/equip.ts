import { PingCompensatedCharacter, ItemName, LocateItemFilters } from "../../../../ALClient/build/index.js"
...
export class FirehazardEquipStrategy implements Strategy<PingCompensatedCharacter> {
    ...
    public equipBest(bot: PingCompensatedCharacter) {
        ...
        if (shouldEquipItem) {
            ...
            if (bot.slots.mainhand && bot.slots.mainhand.name == item && bot.slots.mainhand.level >= itemInfo.level) return // We already have a higher/equivalent one equipped
            return bot.equip(locate, "mainhand")
        }
    }
}